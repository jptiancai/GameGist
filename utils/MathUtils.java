...
public class MathUtils {
	 ...
	 */
	public static boolean shake(double shakeNum) {
		if (shakeNum >= 1) {
			return true;
		}
		if (shakeNum <= 0) {
	...
	/**
	 * 获取num的四舍五入保留bit位小数的值
	 * @param num
	 * @param bit
	 * @return
	 */
	public static double roundNum(double num, int bit) {
		if (bit <= 0) {
			return Math.round(num);
		}
		double bitBase = Math.pow(10, bit);
		return (double)(Math.round(num * bitBase) / bitBase);
	}
